...
def build_autoencoder_latents(
    ...
    output = (
        Path(output_path)
        if output_path
        else cfg.outputs / "features" / f"{cfg.get('autoencoder.output_prefix')}_{latent_size}.parquet"
    )
    ...
    if (
        ...
        log_event("Stage 6 autoencoder", "cache hit", cfg=cfg, latent_size=latent_size, path=output)
        return output

    with stage_timer("Stage 6 autoencoder", "training latent representation", cfg=cfg, latent_size=latent_size):
        import torch
        ...
        total_epochs = int(cfg.get("autoencoder.epochs", 25))
        for epoch in range(total_epochs):
            ...
            if stale_epochs >= patience:
                log_event("Stage 6 autoencoder", "early stopping", cfg=cfg, latent_size=latent_size, epoch=epoch + 1)
                break
        ...
        log_event("Stage 6 autoencoder", "wrote latent artifact", cfg=cfg, latent_size=latent_size, rows=df.height, path=output)
    return output
