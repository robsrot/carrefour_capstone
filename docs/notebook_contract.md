...
Project root used for full paths: `C:\Users\rothl\Desktop\IE Capstone\carrefour_capstone`.
...
| `product_embeddings.parquet` | Parquet | `idarticu:Int64`; `embedding:List[Float32 x 100]`; dev cache 56,375 x 2 | Notebook 3 Cell 13 via `save_embeddings()` | Notebook 3 customer vector builders | `file-mediated (C:\Users\rothl\Desktop\IE Capstone\carrefour_capstone\data\dev\product_embeddings.parquet)` |
| `word2vec_product.model` | gensim `Word2Vec` model | Vector size 100; skip-gram; saved/reloaded from disk | Notebook 3 Cell 11 via `train_word2vec()` | Notebook 3 `save_embeddings()` and sanity checks | `file-mediated (C:\Users\rothl\Desktop\IE Capstone\carrefour_capstone\models\dev\word2vec_product.model)` |
